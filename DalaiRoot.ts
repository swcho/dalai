...
export class Dalai {
  ...
  async query(req: QueryRequest, cb: Callback) {
    ...
    if (req.method === 'installed') {
      const models = await this.installed();
    ...
    const e = await exists(path.resolve(this.home, Core, 'models', Model));
    if (!e) {
      cb(
  ...
  async install(core, ...models) {
    const engine = this.cores[core];

    const venv_path = path.join(this.home, 'venv');
    const ve = await exists(venv_path);
    if (!ve) {
      await this.setup();
    }
    ...
    const models_path = path.resolve(engine.home, 'models');
    const temp_path = path.resolve(this.home, 'tmp');
    const temp_models_path = path.resolve(temp_path, 'models');
    await fs.promises.mkdir(temp_path, { recursive: true }).catch((e) => {
      console.log('1', e);
    });
    // 1. move the models folder to ../tmp
    await fs.promises.rename(models_path, temp_models_path).catch((e) => {
      console.log('2', e);
    });
    // 2. wipe out the folder
    await fs.promises.rm(engine.home, { recursive: true }).catch((e) => {
      console.log('3', e);
    });
    // 3. install engine
    await this.add(core);
    // 4. move back the files inside /tmp
    await fs.promises.rename(temp_models_path, models_path).catch((e) => {
      console.log('4', e);
    });

    // next add the models
    const res = await this.cores[core].add(...models);
    return res;
  }
  async installed() {
    // get cores
    const modelNames: string[] = [];
    for (const core of ['alpaca', 'llama']) {
      const modelsPath = path.resolve(this.home, core, 'models');
  ...
  async add(core) {
     ...
     **************************************************************************************************************/
    const engine = this.cores[core];
    const e = await exists(path.resolve(engine.home));
  ...
  async setup() {
    ...
    if (platform === 'linux') {
      ...
      // for win32 / darwin
      for (const root_pip_path of root_pip_paths) {
    ...
    // 3.3. virtualenv
    const venv_path = path.join(this.home, 'venv');
    for (const root_python_path of root_python_paths) {
      console.log('trying with', root_python_path);
      const code = await this.exec(`${root_python_path} -m venv ${venv_path}`);
      console.log({ code });
    }
    /*
    if (!success) {
